    ...
    <div class="article-tile article-tile-large">
        <div class="col-md-12 col-lg-6">
            <a href="/articles/show/{{ $feature_article->system->codename }}">
            	<img src="{{ $feature_article->teaserImage[0]->url }}" class="article-tile-image">
            </a>
        </div>
        <div class="col-md-12 col-lg-6">
            <div class="article-tile-date">
                {{ date('F j', $feature_article->postDate->getTimeStamp()) }}
            </div>
            <div class="article-tile-content">
                <h2>
                    <a href="/articles/show/{{ $feature_article->system->codename }}">{{ $feature_article->title }}</a>
                </h2>
                <p class="article-tile-text lead-paragraph">
                    {{ $feature_article->summary }}
                </p>
            </div>
        </div>
    </div>
	        ...
	        <div class="col-xs-6 col-md-3">
	            <div>
	                <a href="/cafes" class="ourcoffee-tile-link">
	                    ...
	                    <img class="ourcoffee-tile-image" src="{{ $cafe->photo[0]->url }}" alt="{{ $cafe->photo[0]->description }}">
	                </a>
	            </div>
	        </div>
        @endforeach
